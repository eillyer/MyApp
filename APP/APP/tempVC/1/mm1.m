...
//    }
    ...
    if ([str isEqualToString:@"随机标签"]) {
        randomTagsview *vc = [randomTagsview new];
        [self.navigationController pushViewController:vc animated:YES];
    }
    ...
    if ([str isEqualToString:@"距离传感"]) {
        Julichuangan *vc = [Julichuangan new];
        [self.navigationController pushViewController:vc animated:YES];
    }
//
//    else{
//        [Utility showSYSAlertTitle:@"aaa" text:@"dd" Action:^{
//            DLog(@"asdf");
//        }];
//    }
    ...
    if ([str isEqualToString:@"待续。。。"]) {
        ...
        pppp *view = [pppp CustomView];
        [view ToShowAlertType:topType];
